...
public class ShippingTaskService {
	
	private static Properties props = new Properties();
	private static EntityManager em;

	protected static EntityManager getEM() {
		if (ShippingTaskService.em == null) {
			try {
				...
				EntityManagerFactory emf = Persistence
						.createEntityManagerFactory(ShippingTaskService.props.getProperty("persistence_unit", "pimpl"), props);
				if (emf != null) {
					ShippingTaskService.em = emf.createEntityManager();
					ShippingTaskService.em.setFlushMode(FlushModeType.COMMIT);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		return ShippingTaskService.em;
	}

	static {
		ShippingTaskService.props = new Properties();
		try {
			ShippingTaskService.props.load(new FileInputStream("..\\pimpl.conf"));
		} catch (IOException e) {
			System.err.println("Failed to load configuration file (" + new File("..\\pimpl.conf").getAbsolutePath()
					+ "), continuing with defaults...");
		}

		if (ShippingTaskService.props.getProperty("regenerate", "1").equals("1")) {
			EntityManager em = ShippingTaskService.getEM();
	...
	public Customer getCustomerByName(String name) throws UnknownCustomer {
		try {
			...
			return (Customer)ShippingTaskService.em
	...
	public void updateCustomer(Customer c) throws UpdateFailed {
		try {
			...
			ShippingTaskService.em.getTransaction().begin();
			ShippingTaskService.em.merge(c);
			ShippingTaskService.em.getTransaction().commit();		
		} catch(Exception e) {
			ShippingTaskService.em.getTransaction().rollback();
